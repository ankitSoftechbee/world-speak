...
const WithdrawlSummary = () => {
    ...
    const getIncomeSummary = async () => {
        setLoading(true)
        const response = await requestApi.withdrawlSummary(filter)
                ...
                <Table sx={{ minWidth: 650 }} aria-label="simple table">
                    <TableHead className="bg-gray-100">
                        <TableRow>
                            ...
                            <TableCell align="right"><div className='text-lg font-bold'>Amount</div></TableCell>
                            <TableCell align="right"><div className='text-lg font-bold'>Wallet Transfer</div></TableCell>
                            <TableCell align="right"><div className='text-lg font-bold'>Transaction no.</div></TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {data && data.map((item,index) => (
                            ...
                            >
                                <TableCell align='left'>
                                    {item?.dateOfAction?.split('T')[0] || ''}
                                </TableCell>
                                <TableCell align="right">{item.amount}</TableCell>
                                <TableCell align="right">{item.walletTransfer}</TableCell>
                                <TableCell align="right">{item.transactionNo}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
...
export default WithdrawlSummary